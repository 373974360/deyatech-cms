...
public class UserDataServiceImpl implements UserDataService {
    ...
    public Map<String, Object> getDepartmentUserTreeDataList(UserDataQueryVo queryVo) throws Exception {
        ...
        // 获取用户发稿量数据映射
        Map<String, UserDataVo> userDataMap = getUserDataMap(queryVo);
        if (Objects.nonNull(userDataMap)) {
            ...
            // 计算用户数据
            setUserData(userDataMap, departmentMap, userDepartmentIdSet, fraction, days);
            // 装配树
            assembleTree(tree, userDataMap, departmentMap, userDepartmentIdSet);
            // 计算树的值
            calculateTree(tree, fraction, days);
     ...
     * @return
     */
    private Map<String, UserDataVo> getUserDataMap(UserDataQueryVo queryVo) {
        // 用户数据映射
     ...
     */
    private void setUserData(Map<String, UserDataVo> userDataMap,
     ...
     */
    private void assembleTree(List<UserDataVo> tree,
     ...
     * @param days
     */
    private void calculateTree(List<UserDataVo> tree, NumberFormat fraction, double days) {
        for (UserDataVo userDataVo : tree) {
            long[] childrenValue = getChildrenData(userDataVo.getChildren(), fraction, days);
     ...
     * @param days
     */
    private long[]  getChildrenData(List<UserDataVo> children, NumberFormat fraction, double days) {
     ...
     */
    @Override
    public List<UserDataVo> getUserCatalogDataList(UserDataQueryVo queryVo) throws Exception {
        // 用户数据映射
